Nombre del Proyecto: Destino Cordillera

Integrantes: 
-Damon Murray Pacheco
-Benjamin Juanillo Contreras
-Antonella Araya Gallegos

Descripción del Sitio: Página enfocada al turismo sobre la comuna Destino Cordillera, mostrando atracciones de la comuna para atraer a más personas y expandir el negocio local.

Instrucciones para Ejecutar el Sitio: 

Tecnologías Utilizadas: Visual Studio Code, Git y Github.

Distribución de Responsabilidades: 
.-Creación del repositorio: Benjamin Juanillo.
.-Landing page (index) creada por todos los integrantes.
.-Alojamientos-gastronomia: Antonella Araya.
.-Panoramas con estilo css propio: Damon Murray.
.-Reserva-contacto: Benjamin Juanillo.

Enlace:
https://github.com/bjUAutonoma/Equipo2_Solemne1_web
